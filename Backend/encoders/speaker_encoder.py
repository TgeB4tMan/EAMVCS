...
class SpeakerEncoder:
    ...
    def get_speaker_embedding(self, wav_path: Union[str, Path]) -> np.ndarray:
        ...
        try:
            ...
            # --- IMPROVEMENT: VAD (Voice Activity Detection) ---
            # Trim silence to ensure speaker encoder only sees real speech
            import librosa
            import numpy as np
            ...
            if len(y_trimmed) < 1600: # Less than 0.1s
                logger.warning("Audio too short after VAD, using original")
                y_trimmed = y
            
            # --- IMPROVEMENT: Segmented Averaging (Point 6) ---
            # If audio is long, average embeddings over multiple segments for stability
            duration_sec = len(y_trimmed) / sr
            
            if duration_sec > 5.0:
                logger.info(f"Long audio detected ({duration_sec:.1f}s). Applying segmented averaging.")
                # Split into 5-second windows with 50% overlap
                window_size = 5 * sr
                hop_length = 2.5 * sr
                
                embeddings = []
                for start in range(0, len(y_trimmed) - window_size, int(hop_length)):
                    segment = y_trimmed[start : start + window_size]
                    processed_segment = preprocess_wav(segment)
                    embeddings.append(self.encoder.embed_utterance(processed_segment))
                
                # Add the final segment if not already included
                if len(y_trimmed) >= window_size:
                    final_segment = y_trimmed[-window_size:]
                    processed_final = preprocess_wav(final_segment)
                    embeddings.append(self.encoder.embed_utterance(processed_final))
                
                if embeddings:
                    # Average the embeddings
                    embedding = np.mean(embeddings, axis=0)
                    # Normalize (embeddings should stay on the hypersphere)
                    embedding = embedding / np.linalg.norm(embedding)
                    logger.info(f"Stabilized embedding extracted from {len(embeddings)} segments.")
                else:
                    # Fallback
                    wav = preprocess_wav(y_trimmed)
                    embedding = self.encoder.embed_utterance(wav)
            else:
                # Standard extraction for short clips
                wav = preprocess_wav(y_trimmed)
                embedding = self.encoder.embed_utterance(wav)
