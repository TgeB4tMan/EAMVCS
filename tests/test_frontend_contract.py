...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_generate_uses_json_then_audio_fetch(self):
        ...
        self.assertIn("result.translation_applied", self.script)

    def test_reference_language_and_optional_text_inputs_exist(self):
        self.assertIn('id="refLangSelect"', self.html)
        self.assertIn('id="textLangSelect"', self.html)
        self.assertIn('id="refTextInput"', self.html)

    def test_results_include_emotion_feedback_controls(self):
        self.assertIn('id="predictedEmotion"', self.html)
        self.assertIn('id="emotionConfidence"', self.html)
        self.assertIn('id="submitFeedbackBtn"', self.html)
        self.assertIn("async function submitEmotionFeedback()", self.script)
        self.assertIn("/feedback", self.script)
        self.assertIn("/training-status", self.script)
